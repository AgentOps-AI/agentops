...
def collect_stream_content(stream_response: Any, provider: str) -> List[str]:
    ...
    handlers = {
        ...
        "mistral": lambda event: event.data.choices[0].delta.content if hasattr(event.data.choices[0].delta, 'content') else None,
...
@pytest.mark.vcr()
def test_openai_assistants_provider(openai_client):
    ...
    message2 = openai_client.beta.threads.messages.create(
        thread_id=thread.id,
        role="user",
        content="Also, what is the square root of 144?"
    )
    ...
    # Update thread
    updated_thread = openai_client.beta.threads.update(
        thread.id,
        metadata={"test": "value"}
    )
