...
class TestConcurrentInstrumentation(unittest.TestCase):
    ...
    def test_sequential_spans_same_trace(self):
        ...
        trace_id1 = self._create_simple_span("span1")
        trace_id2 = self._create_simple_span("span2")
    ...
    def test_threadpool_without_context_propagation_creates_separate_traces(self):
        ...
        with self.tracer.start_as_current_span("main_task") as main_span:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                futures = [
                    executor.submit(worker_task, f"task_{i}")
                    for i in range(3)
                ]
        ...
        for worker_trace_id in worker_trace_ids:
            self.assertNotEqual(worker_trace_id, main_trace_id, 
        ...
        self.assertEqual(len(unique_trace_ids), 3, 
                        "Each worker should create a separate trace")
        ...
        for worker_span in worker_spans:
            self.assertIsNone(worker_span.parent, 
                            "Worker spans should be root spans without parent")
    ...
    def test_threadpool_with_manual_context_propagation_shares_trace(self):
        ...
        with self.tracer.start_as_current_span("main_task") as main_span:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                futures = [
                    executor.submit(worker_task_with_context, (f"task_{i}", current_context))
                    for i in range(3)
                ]
                results = [future.result() for future in concurrent.futures.as_completed(futures)]
        ...
        for result in results:
            self.assertEqual(result["trace_id"], main_trace_id,
                           f"Worker task {result['task_id']} should share trace with main span")
            self.assertEqual(result["parent_span_id"], main_span_id,
                           f"Worker task {result['task_id']} should have main span as parent")

        # All worker trace IDs should be the same
        unique_trace_ids = set(worker_trace_ids)
        self.assertEqual(len(unique_trace_ids), 1, 
                        "All workers should share the same trace")
    ...
    def test_mixed_sequential_and_concurrent_spans(self):
        ...
        self.assertEqual(len(set(sequential_trace_ids)), 2, 
                        "Sequential spans should have different traces")
        ...
        self.assertEqual(len(unique_concurrent_traces), 1, 
                        "All concurrent spans should share the same trace")
    ...
    def test_internal_span_processor_with_concurrent_spans(self, mock_logger):
        ...
        try:
            def worker_task_with_context(args) -> str:
                ...
                token = context.attach(ctx)
                try:
                    with self.tracer.start_as_current_span(f"openai.chat.completion_{task_id}") as span:
                        time.sleep(0.01)
            ...
            with self.tracer.start_as_current_span("main_session") as main_span:
                current_context = context.get_current()
            ...
            # The processor should have tracked root spans
            # Note: With proper context propagation, all spans should belong to the same trace
            spans = self.tester.get_finished_spans()
    ...
    def test_performance_impact_of_context_propagation(self):
        ...
        self.assertGreater(time_with * 10, time_without, 
                          "Context propagation should not cause extreme performance degradation")
