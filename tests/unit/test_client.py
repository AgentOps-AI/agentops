...
class TestClient:
    ...
    def test_init_with_custom_exporter(self, mock_simple_processor, mock_otlp_exporter, api_key):
        ...
        session = agentops.init(api_key=api_key, exporter=mock_exporter, auto_start_session=True)
    ...
    def test_init_with_custom_processor(self, mock_get_tracer_provider, api_key):
        ...
        session = agentops.init(api_key=api_key, processor=mock_processor, auto_start_session=True)
    ...
    @mock.patch("agentops.telemetry.session.get_tracer_provider")
    @mock.patch("agentops.telemetry.session.SimpleSpanProcessor")
    def test_processor_takes_precedence_over_exporter(self, mock_simple_processor, mock_get_tracer_provider, api_key):
        ...
        session = agentops.init(
            api_key=api_key, processor=mock_processor, exporter=mock_exporter, auto_start_session=True
        )
    ...
    def test_exporter_and_processor_in_config(self, api_key):
        ...
        # Initialize with both processor and exporter using the public API
        agentops.init(api_key=api_key, processor=mock_processor, exporter=mock_exporter, auto_start_session=False)
