...
class TestValidateTraceSpans:
    ...
    def test_validate_trace_spans_success(self, mock_metrics, mock_details, mock_token):
        ...
        assert "OpenAI Chat Completion" in result["llm_span_names"]
        assert result["metrics"]["total_tokens"] == 100

    @patch('agentops.validation.get_jwt_token')
    @patch('agentops.validation.get_trace_details')
    def test_validate_trace_spans_no_llm(self, mock_details, mock_token):
        """Test validation failure when no LLM spans found."""
        mock_token.return_value = "test-token"
        mock_details.return_value = {
            "spans": [{"span_name": "database.query"}]
        }

        with pytest.raises(ValidationError, match="No LLM spans found"):
            validate_trace_spans(trace_id="test-trace", check_llm=True)

    @patch('agentops.validation.get_jwt_token')
    @patch('agentops.validation.get_trace_details')
    def test_validate_trace_spans_retry(self, mock_details, mock_token):
