...
class TestSessionConfig:
    ...
    def test_session_holds_custom_config_values(self, agentops_config):
        """Test that a session created directly with a config holds that config's values"""
        # Create a custom config
        custom_config = Config()
        custom_config.configure(
            endpoint="https://custom-endpoint.agentops.ai",
            max_wait_time=8000
        )
        
        # Create a session directly with the custom config
        session = Session(config=custom_config)
        
        # Verify that the session holds the custom config values
        assert session.config.endpoint == custom_config.endpoint
        assert session.config.max_wait_time == custom_config.max_wait_time
