...
class ExampleRunner:
    ...
    def run_python_example(self, file_path: Path) -> ExampleResult:
        ...
        try:
            result = subprocess.run(
                ["uv", "run", "python", str(file_path)], cwd=self.repo_root, capture_output=True, text=True, timeout=120
            )
            ...
            return ExampleResult(
                file_path=str(file_path.relative_to(self.repo_root)), success=False, error_message=str(e)
            )
    ...
    def run_notebook_example(self, file_path: Path) -> ExampleResult:
        ...
        try:
            result = subprocess.run(
                ["uv", "run", "jupyter", "nbconvert", "--to", "notebook", "--execute", "--stdout", str(file_path)],
                cwd=self.repo_root,
                capture_output=True,
                text=True,
                timeout=120,
    ...
    def run_all_examples(self, max_workers: int = 4) -> List[ExampleResult]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_example = {executor.submit(self.run_example, example): example for example in examples}
