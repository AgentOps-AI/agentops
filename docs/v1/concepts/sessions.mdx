...
Each session is assigned a unique JWT token that can be used to authenticate session-specific API requests. You can retrieve the JWT token for a session using `agentops.get_session_jwt()`:
...
session = agentops.init()
jwt_token = agentops.get_session_jwt()

# Or get JWT for a specific session
jwt_token = agentops.get_session_jwt(session_id="your-session-id")
```

This JWT token is required alongside your API key when making session-specific API requests.
