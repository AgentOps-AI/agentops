...
Retrieve the JWT token for the current or specified session.

**Parameters**:

- `session_id` (str, optional): The ID of the session to get the JWT for. If not provided, uses the current session.
