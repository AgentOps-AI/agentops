...
- `is_disabled` (bool, optional): Global flag to disable AgentOps. If set to true, no events, sessions, or requests will be sent to the AgentOps service. Default value: False.
...
Enables AgentOps with all features as configured. This sets the `is_disabled` flag to false.
AgentOps **will record** all events, sessions, and requests as expected.

---

### `disable()`

Disables AgentOps completely. This sets the `is_disabled` flag to true.
AgentOps **will stop recording** all events, sessions, or requests.
