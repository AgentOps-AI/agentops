...
Alternatively, to name an agent in runtime, the `@track_agent` decorator also allows for an additional keyword argument in the class constructor. Using the above example, you can create an agent with a dynamic name with:
```python
research_agent = MyAgent(name='ResearchAgent')
```
