...
Need instant answers? Install the Mintlify <strong>MCP Docs Server</strong> and chat with the AgentOps docs directly in Cursor, Windsurf, VS Code, Zed, Claude Code, or any MCP-enabled IDE:

```bash
npx mint-mcp add agentops
```

See the <a href="/v2/usage/mcp-docs">MCP Docs guide</a> for details.
</Note>
