...
- `end_state` (str, optional): The end state for the trace(s). Common values include "Success", "Failure", "Error", "Timeout", etc.
...
- `start_session()`: Legacy function for starting sessions. Use `start_trace()` instead.
