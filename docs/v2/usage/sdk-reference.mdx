...
Ends a previously started AgentOps session.

**Parameters**:

- `session_or_status` (Any, optional): Either a Session object returned by `start_session()`, or status information.
- `**kwargs`: Additional attributes to attach to the session span, such as `end_state` and `end_state_reason`.
...
# End the session with additional status information
agentops.end_session(session, end_state="Success", end_state_reason="Task completed")
```
...
Represents an active tracking session in AgentOps. Instances are created by `start_session()` or automatically with `init()` when `auto_start_session=True`.

**Methods**:

- `create_agent(name=None, agent_id=None, **kwargs)`: Creates an agent span within the session.
- `record(event=None)`: Records events like tool usage within the session.
- `end_session(**kwargs)`: Ends the session, with optional attributes.
