---
title: 'OpenAI Agents SDK Example'
description: 'Using OpenAI Agents SDK with AgentOps for a customer service chatbot'
mode: "wide"
---
...
# Airline Customer Service Agent with OpenAI Agents SDK
...
```python
import agentops
# auto_start_session is False, so we'll start a trace manually later
agentops.init(tags=["customer-service-agent", "openai-agents", "agentops-example"], auto_start_session=False) 
```

Import necessary components from `openai-agents`:
```python
...
import random
import uuid
import sys # sys import was in notebook, keeping for consistency
...
# Start AgentOps trace for this specific workflow
tracer = agentops.start_trace(trace_name="Customer Service Agent") # Matched notebook
```
...
async def main():
    ...
    # Simulate a few turns for the MDX example
    simulated_inputs = [
        "What are the baggage rules?",
        "I want to change my seat.",
        "My confirmation is 123XYZ",
        "I want seat 10A",
        "Thanks!"
    ]
    ...
    for user_input in simulated_inputs:
        ...
        # In a real app, you'd get input: user_input = input("Enter your message: ")
        with trace("Customer service", group_id=conversation_id): # Notebook uses "Customer service"
            ...
            for new_item in result.new_items:
                ...
                if isinstance(new_item, MessageOutputItem):
                    ...
                    print(
                        f"Handed off from {new_item.source_agent.name} to {new_item.target_agent.name}" # Matched notebook
                    )
                elif isinstance(new_item, ToolCallItem):
                    print(f"{agent_name}: Calling a tool") # Matched notebook
                elif isinstance(new_item, ToolCallOutputItem):
                    print(f"{agent_name}: Tool call output: {new_item.output}")
                else:
                    print(f"{agent_name}: Skipping item: {new_item.__class__.__name__}") # Matched notebook
            input_items = result.to_input_list()
            current_agent = result.last_agent
        if user_input.lower() == "thanks!": # End simulation
            break
    
    agentops.end_trace(tracer, end_state="Success") # MDX was correct with end_state


# To run in a Jupyter notebook or a script that can handle asyncio:
# await main() 
# For MDX, we'll just define it. To run, you'd typically use:
# if __name__ == "__main__":
#     asyncio.run(main())
```
To run this example, save it as a Python file and execute it. You will be prompted for user input in the console.
