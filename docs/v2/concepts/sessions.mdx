...
When starting a session manually, you can configure it with:

```python
agentops.start_session(
    name="customer_support_session",  # Optional name for the session
    tags=["customer-support", "tier-1"]  # Optional tags for filtering
)
```
