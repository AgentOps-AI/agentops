...
def setup_session_tracer(sender: Session, **kwargs):
    """When session initializes, create telemetry with a recording span"""
    try:
        # SessionTelemetry will check the session.config for custom exporter/processor settings
        setattr(sender, "telemetry", SessionTracer(sender))
        logger.debug(f"[{sender.session_id}] Session telemetry initialized with recording span")
