                        ...
                        with self.tracer.start_as_current_span(span_name) as span:
                            ...
                            if system_messages:
                                ...
                                if hasattr(system_messages[0], 'content'):
                                    span.set_attribute("gen_ai.request.system_message", safe_str(system_messages[0].content))
                            ...
                            try:
                                ...
                                if hasattr(llm_result, 'usage') and llm_result.usage:
                                    if hasattr(llm_result.usage, 'total_tokens'):
                                        span.set_attribute(SpanAttributes.LLM_USAGE_TOTAL_TOKENS, llm_result.usage.total_tokens)
                                    if hasattr(llm_result.usage, 'prompt_tokens'):
                                        span.set_attribute(SpanAttributes.LLM_USAGE_PROMPT_TOKENS, llm_result.usage.prompt_tokens)
                                    if hasattr(llm_result.usage, 'completion_tokens'):
                                        span.set_attribute(SpanAttributes.LLM_USAGE_COMPLETION_TOKENS, llm_result.usage.completion_tokens)
                    ...
                    with self.tracer.start_as_current_span(span_name) as span:
                        ...
                        if hasattr(result, 'usage') and result.usage:
                            ...
                            if hasattr(result.usage, 'completion_tokens'):
                                span.set_attribute(SpanAttributes.LLM_USAGE_COMPLETION_TOKENS, result.usage.completion_tokens)
