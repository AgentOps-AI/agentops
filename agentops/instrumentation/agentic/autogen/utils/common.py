...
def extract_agent_attributes(instance, args, kwargs) -> Dict[str, Any]:
    ...
    description = kwargs.get("description") or kwargs.get("system_message") or (args[1] if len(args) > 1 else "")
    if description:
...
def create_agent_span(tracer, agent_name: str, operation: str, attribute_manager: SpanAttributeManager):
    ...
    span_name = f"{operation} {agent_name}.workflow"
    span_context = create_span(tracer, span_name, kind=SpanKind.CLIENT, attribute_manager=attribute_manager)
    return span_context
