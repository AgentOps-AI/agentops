    ...
    if return_value is not None:
        ...
        if hasattr(return_value, 'content'):
            ...
            if hasattr(return_value, 'model_provider'):
                attributes["workflow.output.model_provider"] = str(return_value.model_provider) if return_value.model_provider else ""
...
def get_workflow_storage_attributes(
    args: Tuple[Any, ...] = (),
    kwargs: Optional[Dict[str, Any]] = None,
    return_value: Optional[Any] = None,
) -> Dict[str, AttributeValue]:
    ...
    Args:
        ...
        kwargs: Keyword arguments passed to the storage method
        return_value: Return value from the storage method
        
    Returns:
        Dictionary of OpenTelemetry attributes for workflow storage
    """
    attributes = get_common_attributes()
    kwargs = kwargs or {}
    ...
    if args and len(args) > 0:
        ...
        if hasattr(workflow, 'storage') and workflow.storage:
            ...
            if hasattr(workflow.storage, 'mode'):
                attributes["workflow.storage.mode"] = str(workflow.storage.mode)
                
        # Workflow identification for storage context
        if hasattr(workflow, 'workflow_id') and workflow.workflow_id:
            attributes["workflow.storage.workflow_id"] = str(workflow.workflow_id)
        if hasattr(workflow, 'session_id') and workflow.session_id:
            attributes["workflow.storage.session_id"] = str(workflow.session_id)
            
    # Process storage operation result
    if return_value is not None:
        if hasattr(return_value, 'session_id'):
            attributes["workflow.storage.operation_result"] = "success"
            attributes["workflow.storage.result_session_id"] = str(return_value.session_id)
        else:
            attributes["workflow.storage.operation_result"] = "unknown"
