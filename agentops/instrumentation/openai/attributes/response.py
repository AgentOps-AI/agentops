...
def get_response_response_attributes(response: 'Response') -> AttributeMap:
    ...
    attributes = _extract_attributes_from_mapping(
        response.__dict__, RESPONSE_ATTRIBUTES)
    ...
    if response.reasoning:
        attributes.update(_extract_attributes_from_mapping(
            response.reasoning.__dict__, RESPONSE_REASONING_ATTRIBUTES))
...
def get_response_output_attributes(output: List['ResponseOutputTypes']) -> AttributeMap:
    ...
    for i, output_item in enumerate(output):
        if isinstance(output_item, ResponseOutputMessage):
            ...
            attributes.update(_extract_attributes_from_mapping_with_index(
                output_item, RESPONSE_OUTPUT_REASONING_ATTRIBUTES, i))

        elif isinstance(output_item, ResponseFunctionToolCall):
            attributes.update(_extract_attributes_from_mapping_with_index(
                output_item, RESPONSE_OUTPUT_TOOL_ATTRIBUTES, i=i, j=0))
...
def get_response_output_message_attributes(index: int, message: 'ResponseOutputMessage') -> AttributeMap:
    ...
    attributes = _extract_attributes_from_mapping_with_index(
        message, RESPONSE_OUTPUT_MESSAGE_ATTRIBUTES, index)
    ...
    if message.content:
        for i, content in enumerate(message.content):
            if isinstance(content, ResponseOutputText):
                ...
                logger.debug(f"[agentops.instrumentation.openai.response] '{content}' is not a recognized content type.")
...
def get_response_tools_attributes(tools: List['ToolTypes']) -> AttributeMap:
    ...
    for i, tool in enumerate(tools):
        if isinstance(tool, FunctionTool):
            attributes.update(_extract_attributes_from_mapping_with_index(
                tool, RESPONSE_TOOL_ATTRIBUTES, i))
...
def get_response_tool_web_search_attributes(tool: 'WebSearchTool', index: int) -> AttributeMap:
    ...
    return _extract_attributes_from_mapping_with_index(
        tool_data, RESPONSE_TOOL_WEB_SEARCH_ATTRIBUTES, index)
...
def get_response_tool_file_search_attributes(tool: 'FileSearchTool', index: int) -> AttributeMap:
    ...
    return _extract_attributes_from_mapping_with_index(
        tool_data, RESPONSE_TOOL_FILE_SEARCH_ATTRIBUTES, index)
...
def get_response_tool_computer_attributes(tool: 'ComputerTool', index: int) -> AttributeMap:
    ...
    return _extract_attributes_from_mapping_with_index(
        tool_data, RESPONSE_TOOL_COMPUTER_ATTRIBUTES, index)
...
def get_response_usage_attributes(usage: 'ResponseUsage') -> AttributeMap:
    ...
    if hasattr(usage, 'input_tokens_details'):
        ...
        if input_details is None:
            ...
            attributes.update(_extract_attributes_from_mapping(
                input_details.__dict__, RESPONSE_USAGE_DETAILS_ATTRIBUTES))

        elif isinstance(input_details, dict):  # openai-agents often returns a dict for some reason.
            attributes.update(_extract_attributes_from_mapping(
                input_details, RESPONSE_USAGE_DETAILS_ATTRIBUTES))

        else:
            logger.debug(f"[agentops.instrumentation.openai.response] '{input_details}' is not a recognized input details type.")
    ...
    if output_details is None:
        ...
        attributes.update(_extract_attributes_from_mapping(
            output_details.__dict__, RESPONSE_USAGE_DETAILS_ATTRIBUTES))

    else:
        logger.debug(f"[agentops.instrumentation.openai.response] '{output_details}' is not a recognized output details type.")
