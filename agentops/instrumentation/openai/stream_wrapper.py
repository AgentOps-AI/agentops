...
def responses_stream_wrapper(tracer, wrapped, instance, args, kwargs):
    ...
    try:
        ...
        if "input" in kwargs:
            span.set_attribute("llm.openai.request.input", kwargs["input"])
        ...
        if is_streaming:
            ...
            if hasattr(response, "usage"):
                if hasattr(response.usage, "input_tokens"):
                    span.set_attribute(SpanAttributes.LLM_USAGE_PROMPT_TOKENS, int(response.usage.input_tokens))
                if hasattr(response.usage, "output_tokens"):
                    span.set_attribute(SpanAttributes.LLM_USAGE_COMPLETION_TOKENS, int(response.usage.output_tokens))
                if hasattr(response.usage, "total_tokens"):
                    span.set_attribute(SpanAttributes.LLM_USAGE_TOTAL_TOKENS, int(response.usage.total_tokens))

            if hasattr(response, "id"):
                span.set_attribute(SpanAttributes.LLM_RESPONSE_ID, response.id)
            if hasattr(response, "model"):
