...
def extract_tool_definitions(tools: List[Dict[str, Any]]) -> AttributeMap:
    ...
    try:
        ...
        logger.debug(f"[agentops.instrumentation.providers.anthropic] Error extracting tool definitions: {e}")
...
def extract_tool_use_blocks(content_blocks: List[Any]) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        logger.debug(f"[agentops.instrumentation.providers.anthropic] Error extracting tool use blocks: {e}")
        return None
...
def extract_tool_results(content_blocks: List[Any]) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        logger.debug(f"[agentops.instrumentation.providers.anthropic] Error extracting tool results: {e}")
        return None
...
def get_tool_attributes(message_content: List[Any]) -> AttributeMap:
    ...
    try:
        ...
        logger.debug(f"[agentops.instrumentation.providers.anthropic] Error extracting tool attributes: {e}")
