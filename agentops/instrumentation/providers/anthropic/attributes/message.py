    ...
    if return_value:
        try:
            ...
            if isinstance(return_value, Message):
                ...
                logger.debug(
                    f"[agentops.instrumentation.providers.anthropic] Unrecognized return type: {type(return_value)}"
                )
        except Exception as e:
            logger.debug(f"[agentops.instrumentation.providers.anthropic] Error extracting response attributes: {e}")
    ...
    if return_value:
        try:
            if hasattr(return_value, "__class__") and return_value.__class__.__name__ == "Completion":
                ...
                logger.debug(
                    f"[agentops.instrumentation.providers.anthropic] Unrecognized completion return type: {type(return_value)}"
                )
        except Exception as e:
            logger.debug(
                f"[agentops.instrumentation.providers.anthropic] Error extracting completion response attributes: {e}"
            )
...
def get_message_response_attributes(response: "Message") -> AttributeMap:
    ...
    if hasattr(response, "content"):
        try:
            ...
            logger.debug(f"[agentops.instrumentation.providers.anthropic] Error extracting content: {e}")
