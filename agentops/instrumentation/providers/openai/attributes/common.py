...
try:
    ...
    logger.debug(f"[agentops.instrumentation.providers.openai] Could not import OpenAI types: {e}")
...
def get_response_attributes(
    ...
    if return_value:
        if isinstance(return_value, Response):
            ...
            logger.debug(
                f"[agentops.instrumentation.providers.openai] Got an unexpected return type: {type(return_value)}"
            )
