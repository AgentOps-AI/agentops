...
try:
    ...
    logger.debug(f"[agentops.instrumentation.frameworks.openai_agents] Could not import OpenAI Agents SDK types: {e}")
...
def get_response_kwarg_attributes(kwargs: dict) -> AttributeMap:
    ...
    if isinstance(_input, str):
        ...
        logger.debug(
            f"[agentops.instrumentation.providers.openai.response] '{type(_input)}' is not a recognized input type."
        )
...
def get_response_output_attributes(output: List["ResponseOutputTypes"]) -> AttributeMap:
    ...
    for i, output_item in enumerate(output):
        if isinstance(output_item, ResponseOutputMessage):
            ...
            logger.debug(
                f"[agentops.instrumentation.providers.openai.response] '{output_item}' is not a recognized output type."
            )
    ...
    if message.content:
        for i, content in enumerate(message.content):
            if isinstance(content, ResponseOutputText):
                ...
                logger.debug(
                    f"[agentops.instrumentation.providers.openai.response] '{content}' is not a recognized content type."
                )
...
def get_response_tools_attributes(tools: List["ToolTypes"]) -> AttributeMap:
    ...
    for i, tool in enumerate(tools):
        if isinstance(tool, FunctionTool):
            ...
            logger.debug(
                f"[agentops.instrumentation.providers.openai.response] '{tool}' is not a recognized tool type."
            )
...
def get_response_usage_attributes(usage: "ResponseUsage") -> AttributeMap:
    ...
    if hasattr(usage, "input_tokens_details"):
        ...
        if input_details is None:
            ...
            logger.debug(
                f"[agentops.instrumentation.providers.openai.response] '{input_details}' is not a recognized input details type."
            )
    ...
    if output_details is None:
        ...
        logger.debug(
            f"[agentops.instrumentation.providers.openai.response] '{output_details}' is not a recognized output details type."
        )
