	...
	if isinstance(content, list):
		...
		try:
			...
			if extracted_text:
				...
				attributes[MessageAttributes.PROMPT_ROLE.format(i=0)] = "assistant" if getattr(content, "role", "user") == "model" else getattr(content, "role", "user")
		except Exception as e:
...
def _set_response_attributes(attributes: AttributeMap, response: Any) -> None:
	...
	try:
		if hasattr(response, "text"):
			# Accessing response.text may emit warnings when non-text parts are present; still safe to record
			attributes[MessageAttributes.COMPLETION_CONTENT.format(i=0)] = response.text or ""
