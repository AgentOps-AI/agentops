...
class SafeBatchSpanProcessor(BatchSpanProcessor):
    """
    A safer implementation of BatchSpanProcessor that ensures all spans are exported
    during shutdown and handles timeouts gracefully.

    This processor extends the standard BatchSpanProcessor to provide more
    robust handling of span export during shutdown, ensuring that spans aren't
    lost when the application terminates.
    """

    def __init__(self, *args, max_export_batch_size: int = 512,
                 schedule_delay_millis: int = 5000,
                 export_timeout_millis: int = 30000, **kwargs):
        super().__init__(*args,
    ...
    def shutdown(self) -> None:
        """
        Shuts down the processor and ensures all spans are exported.

        This overridden method ensures that all queued spans are exported
        before the processor is fully shut down, with proper timeout handling.
        """
        if self._shutdown:
            return
        ...
        try:
            ...
            timeout_ms = int(self.export_timeout_millis)
            self.force_flush(timeout_ms)
        except Exception as e:
            logger.warning(f"Error during final flush in shutdown: {e}")

        # Call parent shutdown to clean up resources
        super().shutdown()

    def force_flush(self, timeout_millis: Optional[int] = None) -> bool:
        """
        Forces export of all spans in the queue.
        ...
        """
        if self._shutdown:
            logger.warning("Processor is already shut down, flush may not complete")

        try:
            # Use default timeout if None is provided
            actual_timeout = 30000 if timeout_millis is None else int(timeout_millis)
            result = super().force_flush(actual_timeout)
            return result
        except Exception as e:
            logger.warning(f"Error flushing spans: {e}")
            return False
