...
class Client:
    ...
    def init(self, **kwargs) -> Union[Session, None]:
        ...
        # Always fetch JWT token to get project_id before instrumentation
        # This ensures project_id is available for resource attributes
        try:
            token = self.api.v3.fetch_auth_token(self.config.api_key)
            logger.debug("Fetched JWT token for authentication and project_id")
        ...
        if self.config.instrument_llm_calls:
            logger.debug("Initializing instrumentation with project_id in resource attributes")
            instrument_all()
