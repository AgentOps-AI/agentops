...
PROVIDERS = [
    {"name": "OpenAI", "models": ["gpt-4o-mini", "gpt-3.5-turbo"], "env_key": "OPENAI_API_KEY"},
    {
        "name": "Anthropic",
        "models": ["claude-3-haiku-20240307", "claude-3-sonnet-20240229"],
        "env_key": "ANTHROPIC_API_KEY",
    },
    {"name": "Cohere", "models": ["command-nightly", "command"], "env_key": "COHERE_API_KEY"},
]
...
def test_basic_completion(model, provider_name):
    ...
    try:
        ...
        response = litellm.completion(model=model, messages=messages, max_tokens=50, temperature=0.3)
...
def test_creative_writing(model, provider_name):
    ...
    messages = [{"role": "user", "content": "Write a creative two-sentence story about a robot chef."}]

    try:
        print(f"🎨 Creative writing test with {provider_name} ({model})...")
        response = litellm.completion(model=model, messages=messages, max_tokens=100, temperature=0.8)

        content = response.choices[0].message.content
        print(f"📖 Story: {content}")
        print("✅ Creative writing successful!")
        return True
...
def test_reasoning(model, provider_name):
    ...
    try:
        ...
        response = litellm.completion(model=model, messages=messages, max_tokens=150, temperature=0.1)
...
def compare_providers_on_task():
    ...
    task_message = [{"role": "user", "content": "Explain machine learning in exactly one sentence."}]
    ...
    for provider in available_providers:
        ...
        try:
            print(f"\n🔄 {provider['name']} ({model}):")
            response = litellm.completion(model=model, messages=task_message, max_tokens=100, temperature=0.5)
            ...
            results.append({"provider": provider["name"], "model": model, "response": content, "tokens": tokens})
