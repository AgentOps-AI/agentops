...
async def async_completion_example():
    ...
    try:
        ...
        response = await litellm.acompletion(model="gpt-4o-mini", messages=messages, temperature=0.7, max_tokens=150)
...
async def async_streaming_example():
    ...
    try:
        ...
        response = await litellm.acompletion(model="gpt-4o-mini", messages=messages, stream=True, max_tokens=100)
...
async def concurrent_completions_example():
    ...
    tasks = [
        {"name": "Math Problem", "messages": [{"role": "user", "content": "What is 15 * 23?"}]},
        {
            "name": "Creative Writing",
            "messages": [{"role": "user", "content": "Write a one-sentence story about a time traveler."}],
        },
        {"name": "Code Question", "messages": [{"role": "user", "content": "How do you reverse a string in Python?"}]},
    ]
    ...
    async def single_completion(task):
        ...
        try:
            ...
            response = await litellm.acompletion(model="gpt-4o-mini", messages=task["messages"], max_tokens=100)

            content = response.choices[0].message.content
            tokens = response.usage.total_tokens
            print(f"✅ {task['name']} completed ({tokens} tokens)")
            return {"task": task["name"], "response": content, "tokens": tokens}
...
async def async_function_calling_example():
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "tip_percentage": {"type": "number", "description": "The tip percentage (e.g., 15 for 15%)"},
                    },
    ...
    messages = [{"role": "user", "content": "Calculate a 18% tip on a $45.50 bill."}]

    try:
        print("🔧 Making async completion with function calling...")
        response = await litellm.acompletion(model="gpt-4o-mini", messages=messages, tools=tools, max_tokens=150)
