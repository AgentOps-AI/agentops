...
def function_calling_example():
    ...
    messages = [
        {
            "role": "user",
            "content": "What's the weather like in New York and what's the distance between New York and Los Angeles?",
        }
    ]
    ...
    try:
        ...
        response = litellm.completion(
            model="gpt-4o-mini", messages=messages, tools=tools, tool_choice="auto", max_tokens=300
        )
...
def error_handling_example():
    ...
    try:
        response = litellm.completion(
            model="invalid-model-name", messages=[{"role": "user", "content": "Hello"}], max_tokens=50
        )
    ...
    try:
        response = litellm.completion(
            model="gpt-4o-mini", messages=[{"role": "user", "content": "Say hello"}], max_tokens=10, temperature=0.1
        )

        print(f"📝 Response: {response.choices[0].message.content}")
        print("✅ Proper request successful!")
        return True
...
def streaming_with_callbacks_example():
    ...
    messages = [{"role": "user", "content": "Write a short poem about technology and nature."}]
    ...
    try:
        ...
        response = litellm.completion(
            model="gpt-4o-mini", messages=messages, stream=True, max_tokens=200, temperature=0.7
        )
        ...
        print("✅ Streaming with callbacks successful!")
        return True
...
def batch_processing_example():
    ...
    try:
        ...
        for i, task in enumerate(tasks, 1):
            ...
            response = litellm.completion(model="gpt-4o-mini", messages=[task], max_tokens=50, temperature=0.1)

            content = response.choices[0].message.content
            tokens = response.usage.total_tokens

            results.append({"task": task["content"], "response": content, "tokens": tokens})
