...
from agentops.semconv import SpanAttributes, WorkflowSpanKindValues, Meters
...
def wrap_agent_execute_task(tracer, duration_histogram, token_histogram, wrapped, instance, args, kwargs):
    ...
    with tracer.start_as_current_span(
        ...
        attributes={
            SpanAttributes.WORKFLOW_SPAN_KIND: WorkflowSpanKindValues.AGENT.value,
        }
    ) as span:
        try:
            CrewAISpanAttributes(span=span, instance=instance)
...
def wrap_task_execute(tracer, duration_histogram, token_histogram, wrapped, instance, args, kwargs):
    ...
    with tracer.start_as_current_span(
        ...
        kind=SpanKind.CLIENT,
        attributes={
            SpanAttributes.WORKFLOW_SPAN_KIND: WorkflowSpanKindValues.TASK.value,
        }
    ) as span:
        try:
            CrewAISpanAttributes(span=span, instance=instance)
            result = wrapped(*args, **kwargs)
            set_span_attribute(span, SpanAttributes.ENTITY_OUTPUT, str(result))
            span.set_status(Status(StatusCode.OK))
            return result
        except Exception as ex:
            span.set_status(Status(StatusCode.ERROR, str(ex)))
