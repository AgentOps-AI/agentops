...
class AgentsInstrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs):
        ...
        # Monkey patch the AgentOps shutdown method to check for active streaming operations
        try:
            self._patch_agentops_shutdown()
            logger.info("Monkey patched AgentOps shutdown method")
        except Exception as e:
            logger.error(f"Failed to monkey patch AgentOps shutdown method: {e}")
    
    def _patch_agentops_shutdown(self):
        """Monkey patch the AgentOps shutdown method to check for active streaming operations."""
        try:
            import agentops
            
            # Store the original shutdown method
            original_shutdown = agentops.shutdown
            agentops._original_shutdown = original_shutdown
            
            # Add a method to manually flush active streaming operations
            agentops.flush_streaming_operations = flush_active_streaming_operations
            
            @functools.wraps(original_shutdown)
            def instrumented_shutdown(*args, **kwargs):
                global _active_streaming_operations
                
                # Check if there are any active streaming operations
                if _active_streaming_operations:
                    logger.warning(f"[WARNING] Attempting to shut down AgentOps with {len(_active_streaming_operations)} active streaming operations. Delaying shutdown.")
                    
                    # Try to flush active streaming operations
                    try:
                        from opentelemetry.trace import get_tracer_provider, get_current_span
                        ...
                        if hasattr(current_span, "get_span_context"):
                            ...
                            if hasattr(span_context, "trace_id"):
                                ...
                                logger.info(f"[DEBUG] Current trace ID for shutdown: {current_trace_id}")
                            if hasattr(span_context, "span_id"):
                                current_span_id = span_context.span_id
                                logger.info(f"[DEBUG] Current span ID for shutdown: {current_span_id}")
                        
                        # Create attributes for the shutdown span
                        shutdown_attributes = {
                            "service.name": "agentops.agents",
                            "flush_type": "shutdown",
                            "active_operations": len(_active_streaming_operations),
                        ...
                        if current_trace_id:
                            shutdown_attributes[CoreAttributes.TRACE_ID] = current_trace_id
                        
                        # Add parent span ID if available
                        if current_span_id:
                            shutdown_attributes[CoreAttributes.PARENT_ID] = current_span_id
                        
                        # Create a final shutdown span to ensure all metrics are flushed
                        tracer = get_tracer(__name__, __version__, tracer_provider)
                        with tracer.start_as_current_span(
                            name="agents.shutdown.flush",
                            kind=SpanKind.INTERNAL,
                            attributes=shutdown_attributes
                        ) as shutdown_span:
                            logger.info("[DEBUG] Created shutdown flush span")
                            
                            # Add a marker to indicate this is a shutdown flush span
                            shutdown_span.set_attribute("shutdown_marker", "true")
                            
                            # Flush active streaming operations
                            ...
                            if hasattr(tracer_provider, "force_flush"):
                                try:
                                    ...
                                    logger.info("[DEBUG] Successfully flushed shutdown span")
                                except Exception as e:
                                    logger.error(f"[DEBUG] Error flushing shutdown span: {e}")
                    except Exception as e:
                        logger.error(f"[ERROR] Failed to flush active streaming operations: {e}")
                    
                    # Wait for a short time to allow streaming operations to complete
                    for _ in range(10):  # Try for up to 1 second (10 * 0.1s)
                        time.sleep(0.1)
                        if not _active_streaming_operations:
                            logger.info("[DEBUG] All streaming operations completed. Proceeding with shutdown.")
                            break
                    
                    # If there are still active operations, log a warning but proceed
                    if _active_streaming_operations:
                        logger.warning(f"[WARNING] Proceeding with shutdown despite {len(_active_streaming_operations)} active streaming operations.")
                
                # Call the original shutdown method
                return original_shutdown(*args, **kwargs)
            
            # Replace the original shutdown method
            agentops.shutdown = instrumented_shutdown
            
        except Exception as e:
            logger.error(f"Failed to patch AgentOps shutdown method: {e}")
    ...
    def _patch_runner_class(self, tracer_provider):
        ...
        for method_name, original_method in original_methods.items():
            ...
            if method_name == "run_streamed":
                ...
                def instrumented_run_streamed(cls, starting_agent, input, context=None, max_turns=10, hooks=None, run_config=None, _original=original_method, _tracer_provider=tracer_provider):
                                ...
                                try:
                                    ...
                                    with usage_tracer.start_as_current_span(
                                        ...
                                        if hasattr(result, "final_output"):
                                            usage_span.set_attribute(WorkflowAttributes.FINAL_OUTPUT, str(result.final_output)[:1000])
