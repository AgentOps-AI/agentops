...
from agentops.session.exporters import OTLPSpanExporter
...
class AgentsDetailedExporter:
    ...
    def _export_trace(self, trace: AgentsTrace) -> None:
        ...
        with tracer.start_as_current_span(
            ...
            attributes={
                CoreAttributes.WORKFLOW_NAME: trace.name,
    ...
    def _export_span(self, span: AgentsSpan[Any]) -> None:
        ...
        if hasattr(span_data, 'input') and span_data.input:
            attributes[LLMAttributes.INPUT] = str(span_data.input)[:1000]  # Truncate long inputs
        
        if hasattr(span_data, 'output') and span_data.output:
            attributes[LLMAttributes.OUTPUT] = str(span_data.output)[:1000]  # Truncate long outputs
        ...
        if span_type == "Generation" and hasattr(span_data, 'model') and span_data.model:
            attributes[LLMAttributes.MODEL_NAME] = span_data.model
            attributes["gen_ai.llm.model"] = span_data.model  # Standard OpenTelemetry attribute
            ...
            if _agent_token_usage_histogram:
                if 'prompt_tokens' in span_data.usage:
                    _agent_token_usage_histogram.record(
                        ...
                        {
                            "token_type": "input", 
                            "model": attributes.get(LLMAttributes.MODEL_NAME, "unknown"),
                            "gen_ai.llm.model": attributes.get(LLMAttributes.MODEL_NAME, "unknown"),
                            "gen_ai.system": "openai"
                        }
                    )
                    attributes[LLMAttributes.INPUT_TOKENS] = span_data.usage['prompt_tokens']
                ...
                if 'completion_tokens' in span_data.usage:
                    ...
                    attributes[LLMAttributes.OUTPUT_TOKENS] = span_data.usage['completion_tokens']
                
                if 'total_tokens' in span_data.usage:
                    attributes[LLMAttributes.TOTAL_TOKENS] = span_data.usage['total_tokens']
        ...
        for method_name, original_method in original_methods.items():
            ...
            if is_async:
                ...
                async def instrumented_method(cls, starting_agent, input, context=None, max_turns=10, hooks=None, run_config=None, _method_name=method_name, _original=original_method):
                    ...
                    attributes = {
                        "span.kind": AOSpanKind.WORKFLOW_STEP,
                        ...
                        LLMAttributes.MODEL_NAME: model_name,
                        "gen_ai.llm.model": model_name,  # Standard OpenTelemetry attribute
                    ...
                    with tracer.start_as_current_span(
                        ...
                        if hasattr(starting_agent, "model_settings") and starting_agent.model_settings:
                            ...
                            if hasattr(starting_agent.model_settings, "frequency_penalty") and starting_agent.model_settings.frequency_penalty is not None:
                                span.set_attribute(SpanAttributes.LLM_FREQUENCY_PENALTY, starting_agent.model_settings.frequency_penalty)
                            
                            if hasattr(starting_agent.model_settings, "presence_penalty") and starting_agent.model_settings.presence_penalty is not None:
                                span.set_attribute(SpanAttributes.LLM_PRESENCE_PENALTY, starting_agent.model_settings.presence_penalty)
                        
                        try:
                            # Execute the original method
                            result = await _original(starting_agent, input, context, max_turns, hooks, run_config)
                            ...
                            if hasattr(result, "raw_responses") and result.raw_responses:
                                ...
                                for i, response in enumerate(result.raw_responses):
                                    ...
                                    if hasattr(response, "model"):
                                        ...
                                        span.set_attribute(LLMAttributes.MODEL_NAME, model_name)
                                    ...
                                    if hasattr(response, "usage"):
                                        ...
                                        if hasattr(usage, "total_tokens"):
                                            span.set_attribute(f"{LLMAttributes.TOTAL_TOKENS}.{i}", usage.total_tokens)
                                            total_tokens += usage.total_tokens
                                
                                # Set total token counts
                                if total_input_tokens > 0:
                                    span.set_attribute(LLMAttributes.INPUT_TOKENS, total_input_tokens)
                                
                                if total_output_tokens > 0:
                                    span.set_attribute(LLMAttributes.OUTPUT_TOKENS, total_output_tokens)
                                
                                if total_tokens > 0:
                                    span.set_attribute(LLMAttributes.TOTAL_TOKENS, total_tokens)
                ...
                def instrumented_method(cls, starting_agent, input, context=None, max_turns=10, hooks=None, run_config=None, _method_name=method_name, _original=original_method):
                    ...
                    with tracer.start_as_current_span(
                        ...
                        try:
                            # Execute the original method
                            result = _original(starting_agent, input, context, max_turns, hooks, run_config)
